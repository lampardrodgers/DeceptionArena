/**
 * 【v0.1.10 机器人快照，仅供基准对比】
 *
 * 本文件是 `src/ai/bot.ts` 在 v0.1.10（提交 da255d3）时的逐字复制，只改了这段文件头注释、
 * 并在末尾补了一个 `prevBot: BotSide` 导出。它不参与任何应用逻辑，只被 `src/ai/bench.test.ts`
 * 之类的基准测试引用，用来衡量新版机器人相对旧版的进步（头对头、胜率、泄露度量）。
 *
 * 请不要在这里修复 bug 或改进策略——快照一旦漂移就失去了基准的意义。
 * 下面是原文件的说明：
 *
 * 内置算法机器人（不用大模型的和也）。
 *
...
import type { BotSide } from "./sim.js";
...
/**
 * 旧机器人（v0.1.10）作为对局模拟里的一方。
 * 只被测试 / 基准引用；配合 `botAsKaiji("prev", prevBot)` 可以让它坐到开司一侧做头对头。
 */
export const prevBot: BotSide = { select: botSelect, bet: botBet, view: publicView };
